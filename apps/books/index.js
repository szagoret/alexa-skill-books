...
app.intent('getRating', {
    ...
    if (book) {
        message = 'There are ' + book.ratings_count + ' user ratings. The average\
rating is ' + book.average_rating;
    }
    else {
    ...
    request(url, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            xml2js(body, function (error, result) {
                if (!error) {
                    // Extract just the fields that we want to work with.
                    // This minimizes the session footprint and makes it easier to debug.
                    var b = result.GoodreadsResponse.book[0];
                    book = {
                        id: b.id, title: b.title, ratings_count: b.ratings_count,
                        average_rating: b.average_rating, authors: b.authors
                    };
                }
                else {
                    book = { error: error };
...
app.intent('AMAZON.HelpIntent', {
    ...
    message = 'I can tell you details about books from GoodReads. To find a book,\
just say get book, followed by the title. You can also ask who is the author or what \
is the rating.';
    res.say(message).shouldEndSession(false);
});
